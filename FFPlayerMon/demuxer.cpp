...
bool Demuxer::operator()(AVPacket &packet) {
	return av_read_frame(format_context_, &packet) >= 0;			//从媒体文件中获取得到的一个packet可能包含多个(整数个)音频帧或单个视频帧，或者其他类型的流数据。
}

int Demuxer::reset_video_stream() const {
	return av_seek_frame(format_context_, video_stream_index_, 0,AVSEEK_FLAG_FRAME );// AVSEEK_FLAG_ANY
}
